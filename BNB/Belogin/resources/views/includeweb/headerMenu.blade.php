...
<section class=" clearfix main-menu5 menu-main z-depth-1" id="header">
    ...
    <li class="tab col s2 menu-name1"><a  href="{{URL::to('/')}}" id="addNewInvoiceBtn">Home</a></li>
                @if($m2=App\Model\MainManuModel::where('status',1)->where('manu_indexing',2)->first())
                 <li class="tab col s2 menu-name1">
                <a   href="{{URL::to('/')}}/fushaion/{{$m2->id}}" target="_Self">{{$m2->manu_name}}</a>
                </li>
                 @endif
                @if($m6=App\Model\MainManuModel::where('status',1)->where('manu_indexing',6)->first())
                <li class="tab col s2 menu-name1"><a href="{{URL::to('/')}}/fushaion/{{$m6->id}}" target="_Self">{{$m6->manu_name}}</a></li> 
                @endif
                @if($m5=App\Model\MainManuModel::where('status',1)->where('manu_indexing',5)->first())
                 <li class="tab col s2 menu-name3"><a href="{{URL::to('/')}}/fushaion/{{$m5->id}}" target="_Self">{{$m5->manu_name}}</a></li>
                @endif

                
                @if($m3=App\Model\MainManuModel::where('status',1)->where('manu_indexing',3)->first())
                <li class="tab col s2 menu-name2"><a href="" target="_Self">{{$m3->manu_name}}</a></li>
                @endif

                 @if($m4=App\Model\MainManuModel::where('status',1)->where('manu_indexing',4)->first())
                 <li class="tab col s2"><a href="" target="_Self">{{$m4->manu_name}}</a></li>
                 @endif 
            </ul>
